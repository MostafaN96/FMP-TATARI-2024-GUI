...
export class CircularKnittingMachineBussinessmanService {

  urlService: string = "circular-knitting-machine-bussinessman/"
  ...
  selectByManufacture(id: string): Observable<any> {
    const url = `${this._constantsService.BASE_URL}${this.urlService}by-manufacture/${id}`;
    return this._http.get(url,
      {
        headers: new HttpHeaders({
          'authorization': `Bearer ${localStorage.getItem('token')}`
