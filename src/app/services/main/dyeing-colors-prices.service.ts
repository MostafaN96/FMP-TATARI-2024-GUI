...
export class DyeingColorsPricesService {

  urlService: string = "dyeing-colors-prices/"
  ...
  selectByColorId(colorId: string): Observable<any> {
    const url = `${this._constantsService.BASE_URL}${this.urlService}select-by-color/${colorId}`;
    return this._http.get(url,
      {
        headers: new HttpHeaders({
          'authorization': `Bearer ${localStorage.getItem('token')}`
