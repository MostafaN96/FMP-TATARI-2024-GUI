...
import { ReconcilitionRequisitionWcService } from "src/app/services/main/wc/reconcilition-requisition-wc.service";
...
@Component({
  selector: 'app-add-reconciliation-requisition-wc',
  templateUrl: './add-reconciliation-requisition-wc.component.html',
  styleUrls: ['./add-reconciliation-requisition-wc.component.css']
})
export class AddReconciliationRequisitionWcComponent implements OnInit {

  ///////////////////////////////// Form Group & Form Control ////////////////////////////////
  reconcilitionRequisitionWCForm = new FormGroup({
  ...
  addItem() {
    const control = <FormArray>this.reconcilitionRequisitionWCForm.get('items');
  ...
  selectConsigment(event: { itemData: any; }, row: FormGroup, index) {
    if (!this.consigments[index].includes(event.itemData)) {
      ...
      this._reportWcService.selectPriceByFabricByConsigmentManufacturingInWc(row.controls['fabricId'].value!, event.itemData.id).subscribe((response: any) => {
        ...
        this.listFabricPricesDollar[index] = [this._sharedComponentService.getAvgPriceDynamic(this.fabricsDetails, 'quantity', 'price_dollar'), this._sharedComponentService.getAvgInputesPriceDynamic(this.fabricsDetails, 'quantity', 'price_dollar'), this.fabricsDetails[0].latest_manufacturing_price, this.fabricsDetails[0].latest_price]
  ...
  async onReconcilitionRequisitionWc() {
    this.reconcilitionRequisitionWCForm.markAllAsTouched();
    if (this.reconcilitionRequisitionWCForm.valid) {
      if (this._quantityOccurrencesValidationService.validateCurrentQuantityTwoItemsReconciliation(
        this.reconcilitionRequisitionWCForm.controls['items'].value, this.reconcilitionRequisitionWCForm.controls['items'].value, 
        'fabricId', 'fabricId',
        'consigmentManufacturingId', 'consigmentManufacturingId',
        'quantity', 'fabricCode', this.currentQuantity, "inputOutput")) {
      const formGroup = await this._sharedComponentService.deleteControlsOfFormArray(this.reconcilitionRequisitionWCForm, 'items',
          ['fabricCode'])
      this._constantsService.spinner.show()
      this._reconcilitionRequisitionWcService.add(formGroup.value).subscribe(response => {
        this._constantsService.spinner.hide();
        if (response.msg == "data inserted") {
          this._constantsService.successAddMessage()
          this._sharedComponentService.openNewTab(`${this._constantsService.ROUTING_MAIN_LINKS[0]}${this._constantsService.ROUTING_LINKS[38]}/details`, { id: response.id });
