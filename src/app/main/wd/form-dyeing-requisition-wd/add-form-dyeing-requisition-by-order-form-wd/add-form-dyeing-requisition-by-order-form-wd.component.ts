...
@Component({
  selector: 'app-add-form-dyeing-requisition-by-order-form-wd',
  templateUrl: './add-form-dyeing-requisition-by-order-form-wd.component.html',
  styleUrls: ['./add-form-dyeing-requisition-by-order-form-wd.component.css']
})
export class AddFormDyeingRequisitionByOrderFormWdComponent implements OnInit {
  ...
  getData() {
    this.route.queryParams.subscribe(params => {
      ...
      this.addRequisitionForm.controls.dyeingId.setValue(this.selectedData.dyeing_id)
      this.addRequisitionForm.controls.sellerId.setValue(this.selectedData.seller_id)
    })
    ...
    // Select Dyeing
    this.dyerName = this.selectedData.dyeing_name
      ...
      this.selectedDyeingId = this.selectedData.dyeing_id
      ...
      this._dyeingServicesService.selectByDeying(this.selectedData.dyeing_id).subscribe((response: any) => {
        this.dyeingServicesData = response
      })

      // Select Seller
      this._dyeingOrderDetailsWdService.selectOrdersBySeller(this.selectedData.seller_id).subscribe((response: any) => {
        this.dyeingOrders = response
        this.sellerName = this.selectedData.seller_name
  ...
  async onAddRequisition() {
    ...
    if (this.addRequisitionForm.valid && this.addRequisitionForm.get('items')!['controls'].length > 0) {
      if (this._quantityOccurrencesValidationService.validateQuantityDynamic(
        ...
        this._formDyeingRequisitionDetailsWdService.addByOrder(formGroup.value).subscribe(response => {
          this._constantsService.spinner.hide();
          if (response.msg == "data inserted") {
            this._constantsService.successAddMessage()
            this._sharedComponentService.reloadPageWithParams(this.selectedData?.requisition_id);
          }
          else{
