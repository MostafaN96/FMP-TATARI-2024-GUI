...
@Component({
  selector: 'app-wd-form-dyeing-order-requisition-update',
  templateUrl: '../update-form-dyeing-requisition-wd/update-form-dyeing-requisition-wd.component.html',
  styleUrls: ['./wd-form-dyeing-order-requisition-update.component.css']
})
export class WdFormDyeingOrderRequisitionUpdateComponent implements OnInit {
  ...
  inputDyeingWdForm: FormGroup = new FormGroup({
    ...
    dyeingColorsPricesId: new FormControl("", [Validators.required]),
  ...
  async onUpdate() {
    ...
    if (this.inputDyeingWdForm.valid) {
      ...
      this._formDyeingRequisitionDetailsWdService.updateByOrder(formGroup.value, this.selectedData.id).subscribe((response: any) => {
