...
@Component({
  selector: 'app-add-transition-between-wh-requisition-wa',
  templateUrl: './add-transition-between-wh-requisition-wa.component.html',
  styleUrls: ['./add-transition-between-wh-requisition-wa.component.css']
})
export class AddTransitionBetweenWhRequisitionWaComponent implements OnInit {
  ...
  selectConsigmentYarn(event: { itemData: any; }, row: FormGroup, index: number) {
    ...
    if (this.fromConsigmentsYarns[index][indexData] !== event.itemData) {
      ...
      row.controls['newConsigmentYarnNumber'].setValue("")
  ...
  async onAddTransitionIndustriesRequisition() {
    ...
    if (this.addtransitionIndustriesRequisitionForm.valid) {
      ...
      this._transitionBetweenWhRequisitionWaService.add(this.addtransitionIndustriesRequisitionForm.value).subscribe(response => {
