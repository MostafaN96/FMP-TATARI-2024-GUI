...
@Component({
  selector: 'app-update-transition-between-wh-requisition-we',
  templateUrl: './update-transition-between-wh-requisition-we.component.html',
  styleUrls: ['./update-transition-between-wh-requisition-we.component.css']
})
export class UpdateTransitionBetweenWhRequisitionWeComponent {
  ...
  onUpdate() {
    ...
    if (this.transitionBetweenWhRequisitionWeForm.valid) {
      ...
      this._transitionBetweenWhRequisitionDetailsWeService.update(this.transitionBetweenWhRequisitionWeForm.value, this.selectedData.id).subscribe((response: any) => {
