...
export class WbTransportWbWaRequisitionDetailsService {

  urlService: string = "wb-transport-wb-wa-requisition-details/"
  ...
  selectWithFabricManufacturedByRequisitionId(id: string): Observable<any> {
    const url = `${this._constantsService.BASE_URL}${this.urlService}select-with-fabric-manufactured/${id}`;
    return this._http.get(url,
      {
        headers: new HttpHeaders({
          'authorization': `Bearer ${localStorage.getItem('token')}`
