...
@Component({
  selector: 'app-add-form-dyeing-requisition-form-wd',
  templateUrl: './add-form-dyeing-requisition-form-wd.component.html',
  styleUrls: ['./add-form-dyeing-requisition-form-wd.component.css']
})
export class AddFormDyeingRequisitionFormWdComponent implements OnInit {
  ...
  initItem(data: any, index: number) {
    return new FormGroup({
      ...
      parentWcFabricOrderRequisitionId: new FormControl(data.parent_wc_fabric_order_requisition_id, [Validators.required]),
  ...
  selectColorCategory(event: { itemData: any; }, row: FormGroup, index) {
    if (!this.colorCategories.includes(event.itemData)) {
      ...
      this._colorService.selectByCategoryAndDeyingByDyedFabricByFabricOrder(this.selectedData?.dyeing_id, event.itemData.id, row.controls['dyedFabricId'].value, row.controls['parentWcFabricOrderRequisitionId'].value).subscribe((response: any) => {
  ...
  async onAddRequisition() {
    ...
    if (this.addRequisitionForm.valid) {
      if (this._quantityOccurrencesValidationService.validateQuantityDynamic(
        ...
        this._formDyeingRequisitionDetailsWdService.add(formGroup.value).subscribe(response => {
