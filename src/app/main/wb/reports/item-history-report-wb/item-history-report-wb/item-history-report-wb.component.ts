...
export class ItemHistoryReportWbComponent implements OnInit {
  ...
  public columnDefs: MyColDef[] = [
    ...
    { headerName: 'رقم الرسالة', field: 'consigment_yarn_number', filter: 'agSetColumnFilter', filterParams: { excelMode: 'windows' }, excludeFromFooter: true },
    { headerName: 'الطلبية', field: 'wa_yarn_order_requisition_name', filter: 'agSetColumnFilter', filterParams: { excelMode: 'windows' }, excludeFromFooter: true },
    {
    ...
    { headerName: 'آخر سعر دولار', field: 'latest_price_dollar', valueFormatter: this.num2, hide: !this.isShowLatestPriceDollar || !this.canViewPrices(), excludeFromFooter: true, type: 'numericColumn' },
    {
      ...
      cellRenderer: (p: any) => {
        ...
        const queryParams = new URLSearchParams({
          ...
          yarnLotId: p.data.yarn_lot_id,
          yarnLotCode: p.data.yarn_lot_code,
          consigmentYarnId: p.data.consigment_yarn_id,
          consigmentYarnNumber: p.data.consigment_yarn_number,
          yarnOrderId: p.data.wa_yarn_order_requisition_id
        }).toString();
  ...
  private loadData(payload: { isShowClosedBalances: boolean }) {
    ...
    this._reportWbService.selectInverntoryDetails(payload)
      .pipe(
        finalize(() => {
