...
@Component({
  selector: 'app-bar-chart-report-by-item-we',
  templateUrl: './bar-chart-report-by-item-we.component.html',
  styleUrls: ['./bar-chart-report-by-item-we.component.css']
})
export class BarChartReportByItemWeComponent implements OnInit {
  ...
  getData() {
    this._fabricService.selectDyerDyeing("dyed").subscribe((response: any) => {
  ...
  getSelectedItem(data: { itemData: any; }) {
    // Chart
    this._reportWeService.dyeingReportByFabric(data.itemData.id).subscribe((response: any) => {
