    ...
    if (this.yarns[index][indexData] !== event.itemData) {
      ...
      this._reportWbService.selectPriceInWb(event.itemData.id, this.transportWbWaRequisitionForm.controls['industryId'].value!).subscribe((response: any) => {
        ...
        if (this.yarnsDetails && this.yarnsDetails.length > 0) {
          ...
          row.controls['price'].setValue(String(this.yarnsDetails[0].latest_price ?? 0))
          row.controls['priceDollar'].setValue(String(this.yarnsDetails[0].latest_price_dollar ?? 0))
        }
      })
    }
