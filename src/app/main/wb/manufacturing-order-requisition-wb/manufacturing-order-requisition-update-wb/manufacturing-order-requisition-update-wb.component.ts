...
@Component({
  selector: 'app-manufacturing-order-requisition-update-wb',
  templateUrl: './manufacturing-order-requisition-update-wb.component.html',
  styleUrls: ['./manufacturing-order-requisition-update-wb.component.css']
})
export class ManufacturingOrderRequisitionUpdateWbComponent implements OnInit {
  ...
  onUpdate() {
    ...
    if (this.manufacturingOrderWdForm.valid) {
      ...
      this._manufacturingOrderRequisitionDetailsWbService.update(this.manufacturingOrderWdForm.value, this.selectedData.id).subscribe((response: any) => {
  ...
  closeOrder(data) {
    this._constantsService.spinner.show()
    this._manufacturingOrderRequisitionDetailsWbService.closeOrder({}, data.id).subscribe((response: any) => {
      this._constantsService.spinner.hide();
      if (response.msg == "data updated") {
        this._constantsService.successUpdateMessage()
        this._sharedComponentService.reloadPageWithParams(this.requisitionId);
  ...
  openOrder(data) {
    this._constantsService.spinner.show()
    this._manufacturingOrderRequisitionDetailsWbService.openOrder({}, data.id).subscribe((response: any) => {
      this._constantsService.spinner.hide();
      if (response.msg == "data updated") {
        this._constantsService.successUpdateMessage()
        this._sharedComponentService.reloadPageWithParams(this.requisitionId);
