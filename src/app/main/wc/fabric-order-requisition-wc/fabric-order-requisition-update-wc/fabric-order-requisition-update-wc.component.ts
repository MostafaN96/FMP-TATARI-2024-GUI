...
@Component({
  selector: 'app-fabric-order-requisition-update-wc',
  templateUrl: './fabric-order-requisition-update-wc.component.html',
  styleUrls: ['./fabric-order-requisition-update-wc.component.css']
})
export class FabricOrderRequisitionUpdateWcComponent implements OnInit {
  ...
  onUpdate() {
    ...
    if (this.manufacturingOrderWdForm.valid) {
      ...
      this._fabricOrderRequisitionDetailsWcService.update(this.manufacturingOrderWdForm.value, this.selectedData.id).subscribe((response: any) => {
  ...
  closeOrder(data) {
    this._constantsService.spinner.show()
    this._fabricOrderRequisitionDetailsWcService.closeOrder({}, data.id).subscribe((response: any) => {
      this._constantsService.spinner.hide();
      if (response.msg == "data updated") {
        this._constantsService.successUpdateMessage()
        this._sharedComponentService.reloadPageWithParams(this.requisitionId);
  ...
  openOrder(data) {
    this._constantsService.spinner.show()
    this._fabricOrderRequisitionDetailsWcService.openOrder({}, data.id).subscribe((response: any) => {
      this._constantsService.spinner.hide();
      if (response.msg == "data updated") {
        this._constantsService.successUpdateMessage()
        this._sharedComponentService.reloadPageWithParams(this.requisitionId);
