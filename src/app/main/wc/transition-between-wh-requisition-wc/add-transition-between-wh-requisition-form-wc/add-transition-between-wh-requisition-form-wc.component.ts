...
@Component({
  selector: 'app-add-transition-between-wh-requisition-form-wc',
  templateUrl: './add-transition-between-wh-requisition-form-wc.component.html',
  styleUrls: ['./add-transition-between-wh-requisition-form-wc.component.css']
})
export class AddTransitionBetweenWhRequisitionFormWcComponent implements OnInit {
  ...
  getSelectConsigmentManufacturingQuantityByFabricByWarehouseWc(warehouseId: string, fabricId: string, fabricOrderId: string, index) {
  ...
  selectFabricOrder(event: { itemData: any; }, row: FormGroup, index) {
    ...
    if (this.fabricOrder[indexData] !== event.itemData) {
      ...
      this._fabricService.selectByWarehouseWc(row.controls['fromWarehouseId'].value!, event.itemData.id).subscribe((response: any) => {
  ...
  async onAddTransitionIndustriesRequisition() {
    ...
    if (this.addtransitionIndustriesRequisitionForm.valid) {
      ...
      this._transitionBetweenWhRequisitionDetailsWcService.add(this.addtransitionIndustriesRequisitionForm.value).subscribe(response => {
