...
@Component({
  selector: 'app-add-transport-wc-wd-requisition-wc',
  templateUrl: './add-transport-wc-wd-requisition-wc.component.html',
  styleUrls: ['./add-transport-wc-wd-requisition-wc.component.css']
})
export class AddTransportWcWdRequisitionWcComponent implements OnInit {
  ...
  selectFabric(event: { itemData: any; }, row: FormGroup, index) {
    ...
    if (this.fabrics[indexData] !== event.itemData) {
      ...
      this._wcService.selectConsigmentManufacturingQuantityByWarehouseByFabricWc(this.transportWcWdForm.controls['warehouseId'].value!, event.itemData.id).subscribe((response: any) => {
  ...
  async onTransportWcWdRequisition() {
    ...
    if (this.transportWcWdForm.valid) {
      if (this._quantityOccurrencesValidationService.validateCurrentQuantityTwoItems(
        ...
        this._transportWcWdService.add(formGroup.value).subscribe(response => {
          this._constantsService.spinner.hide();
          if (response.msg == "data inserted") {
            this._constantsService.successAddMessage()
            this._sharedComponentService.openNewTab(`${this._constantsService.ROUTING_MAIN_LINKS[0]}${this._constantsService.ROUTING_LINKS[51]}/details`, { id: response.id });
