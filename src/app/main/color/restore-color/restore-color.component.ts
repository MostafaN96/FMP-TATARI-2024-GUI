...
@Component({
  selector: 'app-restore-color',
  templateUrl: './restore-color.component.html',
  styleUrls: ['./restore-color.component.css']
})
export class RestoreColorComponent implements OnInit {
  ...
  getData() {
    this._colorService.selectAllDeleted().subscribe((response: any) => {
      this.color = response
  ...
  restore() {
    ...
    this._colorService.restore(this.selectedData).subscribe(response => {
