...
@Component({
  selector: 'app-wd-reconciliation-requisition-form-add-details',
  templateUrl: './wd-reconciliation-requisition-form-add-details.component.html',
  styleUrls: ['./wd-reconciliation-requisition-form-add-details.component.css']
})
export class WdReconciliationRequisitionFormAddDetailsComponent implements OnInit {
  ...
  public onFilteringFabricName(e: any) {
    ...
    predicate = predicate.or('fabric_dyeing_code', 'contains', e.text);
    var query = new Query();
  ...
  getData() {
    ...
    this.reconcilitionRequisitionForm.controls.dyeingId.setValue(this.selectedData[0]['dyeing_id'])

    this._wdService.selectQuantityByDyeingWd(this.selectedData[0]['dyeing_id']).subscribe((response: any) => {
  ...
  selectFabric(event: { itemData: any; }, row: FormGroup, index) {
    ...
    if (this.fabrics[indexData] !== event.itemData) {
      ...
      this._wdService.selectConsigmentDyeingQuantityByFabricByDyeingWd(event.itemData.fabric_id, this.reconcilitionRequisitionForm.controls['dyeingId'].value!).subscribe((response: any) => {
  ...
  async onReconcilitionRequisition() {
    ...
    if (this.reconcilitionRequisitionForm.valid) {
      ...
      this._reconcilitionRequisitionDetailsWdService.add(formGroup.value).subscribe(response => {
