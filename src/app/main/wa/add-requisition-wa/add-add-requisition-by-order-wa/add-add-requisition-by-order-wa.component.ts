...
@Component({
  selector: 'app-add-add-requisition-by-order-wa',
  templateUrl: './add-add-requisition-by-order-wa.component.html',
  styleUrls: ['./add-add-requisition-by-order-wa.component.css']
})
export class AddAddRequisitionByOrderWaComponent implements OnInit {
  ...
  getData() {
    ...
    this._addPurchaseOrderWaService.selectAll("opened").subscribe((response: any) => {
  ...
  getYarnsOrderData(yarnOrderRequisition) {

    this._addPurchaseOrderDetailsWaService.yarnsOfPurchaseOrderWa(yarnOrderRequisition).subscribe((response: any) => {
  ...
  selectRequisitionsOrderName(event: { itemData: any; }) {
    ...
    if (this.requisitionsOrder.includes(event.itemData)) {
      this.addRequisitionForm.controls['orderId'].setValue(event.itemData.id)
      this.getYarnsOrderData(event.itemData.id)
    } else {
      this.addRequisitionForm.controls['orderId'].setValue("")
    }
  }
  ...
  async onAddRequisition() {
    ...
    if (this.addRequisitionForm.valid) {
      ...
      this._addRequisitionService.addByOrder(this.addRequisitionForm.value).subscribe(response => {
        this._constantsService.spinner.hide();
        if (response.msg === "data inserted") {
          this._constantsService.successAddMessage()
          this._sharedComponentService.openNewTab(`${this._constantsService.ROUTING_MAIN_LINKS[0]}${this._constantsService.ROUTING_LINKS[174]}`, { id: response.id });
