...
export class UpdateColorComponent implements OnInit {
  ...
  ngOnChanges() {
    this.colorForm.controls['name'].setValue(this.selectedData?.name)
  ...
  onUpdate() {
    ...
    this._colorService.update(this.colorForm.value,this.selectedData.id).subscribe((response: any) =>{
