...
export class ColorCategoryService {
  ...
  urlService: string = "color-category/"
  ...
  selectDyersAndRequisitionsColorCategoryOfFabrics(fabricId: string, supplierId: string): Observable<any> {
    const url = `${this._constantsService.BASE_URL}${this.urlService}${fabricId}/${supplierId}`;
    return this._http.get(url,
      {
        headers: new HttpHeaders({
          'authorization': `Bearer ${localStorage.getItem('token')}`
