...
@Component({
  selector: 'app-update-add-requisition-wa',
  templateUrl: './update-add-requisition-wa.component.html',
  styleUrls: ['./update-add-requisition-wa.component.css']
})
export class UpdateAddRequisitionWaComponent implements OnInit {
  ...
  changePrice(type) {
    if(type == "priceEG") {
      ...
      this.addRequisitionWaForm.controls['price'].setValue(this._sharedComponentService.calcDollarToEgp(this.addRequisitionWaForm.controls['priceDollar'].value))
    }
  }
  ...
  onUpdate() {
    ...
    if (this.addRequisitionWaForm.valid) {
      ...
      this._waAddRequisitionDetailsService.update(this.addRequisitionWaForm.value, this.selectedData.id).subscribe((response: any) => {
